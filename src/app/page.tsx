...
export default function StartPage() {
  ...
  useEffect(() => {
    ...
    const safari = /safari/i.test(ua) && !/chrome|crios|fxios|kakaotalk/i.test(ua);
    setIsIOS(ios);
    setIsSafari(safari);

    /* ── 카카오 인앱 Android만 외부브라우저로 이동 ── */
        ...
        <div style={{ position: 'relative', zIndex: 10, flex: 1, background: 'rgba(255,255,255,0.97)', margin: '80px 0 0', borderRadius: '28px 28px 0 0', padding: '28px 24px 40px', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: 24 }}>
            ...
            <p style={{ fontSize: 12, color: '#9CA3AF', lineHeight: 1.7 }}>
              4자리 공용 비밀번호를 입력해 주세요<br />
              모르시는 분은 <strong style={{ color: '#1B3F7B' }}>임원일 과장</strong>에게 문의
            </p>
          </div>
          ...
          <button onClick={handleKakaoLogin} disabled={kakaoLoading || pin.length < 4 || !!pinError}
            ...
            {kakaoLoading
              ...
              : <>
                  <svg width="20" height="20" viewBox="0 0 24 24" fill={pin.length === 4 && !pinError ? '#191919' : '#9CA3AF'}>
    ...
    <div style={{ ...F, minHeight: '100dvh', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'relative', zIndex: 10, minHeight: '100dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px 20px' }}>
        ...
        <div style={{ position: 'relative', width: 120, height: 120, marginBottom: 24 }}>
          <svg style={{ position: 'absolute', inset: -20, width: 160, height: 160, animation: 'spin1 3s linear infinite' }} viewBox="0 0 160 160">
            <defs><linearGradient id="gold1" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor="#c9a84c" stopOpacity="0"/><stop offset="30%" stopColor="#ffe87a" stopOpacity="1"/><stop offset="60%" stopColor="#c9a84c" stopOpacity="1"/><stop offset="100%" stopColor="#8B6914" stopOpacity="0"/></linearGradient></defs>
          ...
          <svg style={{ position: 'absolute', inset: -28, width: 176, height: 176, animation: 'spin2 5s linear infinite' }} viewBox="0 0 176 176">
            <defs><linearGradient id="gold2" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stopColor="#ffe87a" stopOpacity="0"/><stop offset="40%" stopColor="#c9a84c" stopOpacity="0.8"/><stop offset="70%" stopColor="#ffe87a" stopOpacity="0.6"/><stop offset="100%" stopColor="#c9a84c" stopOpacity="0"/></linearGradient></defs>
          ...
          <svg style={{ position: 'absolute', inset: -14, width: 148, height: 148, animation: 'spin3 4s linear infinite' }} viewBox="0 0 148 148">
            <defs><linearGradient id="gold3" x1="100%" y1="0%" x2="0%" y2="100%"><stop offset="0%" stopColor="#c9a84c" stopOpacity="0"/><stop offset="50%" stopColor="#ffe87a" stopOpacity="0.9"/><stop offset="100%" stopColor="#c9a84c" stopOpacity="0"/></linearGradient></defs>
      ...
      {showInstallGuide && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 100, display: 'flex', alignItems: 'flex-end' }}
          ...
          <div style={{ background: '#fff', borderRadius: '24px 24px 0 0', padding: '28px 20px 50px', width: '100%', fontFamily: "'Apple SD Gothic Neo','Noto Sans KR',sans-serif" }}
            onClick={e => e.stopPropagation()}>

            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
              <div style={{ width: 52, height: 52, borderRadius: 14, background: '#1B3F7B', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, flexShrink: 0 }}>📱</div>
              <div>
            ...
            {isIOS ? (
              isSafari ? (
                /* ── Safari에서 접속 중 ── */
                <div>
                  <p style={{ fontSize: 12, fontWeight: 700, color: '#1B3F7B', marginBottom: 10 }}>📌 홈 화면에 추가하는 방법</p>
                  {[
                    ...
                    <div key={item.step} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: '#f8fafc', borderRadius: 12, marginBottom: 8 }}>
                      <div style={{ width: 32, height: 32, borderRadius: '50%', background: '#1B3F7B', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 800, flexShrink: 0 }}>
                        {item.step}
                      </div>
                  ...
                  <div style={{ background: '#f0fdf4', border: '1px solid #86efac', borderRadius: 10, padding: '10px 14px', marginTop: 8, fontSize: 12, color: '#15803d', lineHeight: 1.6 }}>
                    ✅ 지금 Safari로 접속 중이에요! 바로 설치하실 수 있어요.
                  </div>
                </div>
              ) : (
                /* ── Safari 아닌 iOS 브라우저 (카카오 인앱, 크롬 등) ── */
                <div>
                  <div style={{ background: '#fef3c7', border: '1px solid #fde68a', borderRadius: 12, padding: '14px 16px', marginBottom: 16 }}>
                    <p style={{ fontSize: 13, fontWeight: 700, color: '#92400e', marginBottom: 6 }}>⚠️ Safari에서만 설치 가능해요</p>
                    <p style={{ fontSize: 12, color: '#78350f', lineHeight: 1.7 }}>
                      아이폰 홈 화면 추가는 <strong>Safari 브라우저</strong>에서만 가능해요.<br />
                  ...
                  <button
                    ...
                    style={{ width: '100%', background: '#f1f5f9', border: 'none', borderRadius: 12, padding: '11px', fontSize: 13, fontWeight: 600, color: '#475569', cursor: 'pointer', marginTop: 8, fontFamily: 'inherit' }}>
                    📋 주소 복사하기
                  </button>
                </div>
              )
            ...
            <button onClick={() => setShowInstallGuide(false)}
              style={{ width: '100%', background: '#f1f5f9', border: 'none', borderRadius: 12, padding: 14, fontSize: 15, fontWeight: 700, color: '#64748b', cursor: 'pointer', marginTop: 16, fontFamily: 'inherit' }}>
              닫기
            </button>
          </div>
        </div>
