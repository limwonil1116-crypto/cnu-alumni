...
interface AlumniDetail {
  ...
  admission_year?: number; graduation_year?: number;
  phone?: string; email?: string; company?: string; job_title?: string;
  region?: string; address?: string; bio?: string;
  photo_url?: string; card_image_url?: string; profile_id?: string;
  organization?: string; office_phone?: string; fax?: string; profile_email?: string;
}

const ORG_LIST = [
  '한국농어촌공사', '농림축산식품부', '충남도청', '세종특별자치시', '대전광역시',
  '천안시', '공주시', '보령시', '아산시', '서산시', '논산시', '계룡시', '당진시',
  '금산군', '부여군', '서천군', '청양군', '홍성군', '예산군', '태안군',
];

const ORG_LOGO: Record<string, string> = {
  '한국농어촌공사': '/logos/krc.png', '농림축산식품부': '/logos/mafra.png',
  '충남도청': '/logos/chungnam.png', '세종특별자치시': '/logos/sejong.png',
  '대전광역시': '/logos/daejeon.png', '천안시': '/logos/cheonan.png',
  '공주시': '/logos/gongju.png', '보령시': '/logos/boryeong.png',
  '아산시': '/logos/asan.png', '서산시': '/logos/seosan.png',
  '논산시': '/logos/nonsan.png', '계룡시': '/logos/gyeryong.png',
  '당진시': '/logos/dangjin.png', '금산군': '/logos/geumsan.png',
  '부여군': '/logos/buyeo.png', '서천군': '/logos/seocheon.png',
  '청양군': '/logos/cheongyang.png', '홍성군': '/logos/hongseong.png',
  '예산군': '/logos/yesan.png', '태안군': '/logos/taean.png',
};
...
function saveContact(alumni: AlumniDetail) {
  ...
  const vcard = [
    'BEGIN:VCARD', 'VERSION:3.0', `FN:${alumni.name}`, `N:${alumni.name};;;`,
...
async function extractCardInfo(file: File): Promise<{
  ...
  company?: string; job_title?: string; office_phone?: string; fax?: string;
}> {
...
export default function ProfileDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchData = async () => {
    ...
    if (data) {
      ...
      setAlumni({
        id: data.id, name: data.name, department: (data as any).department_name || '',
        admission_year: data.admission_year, graduation_year: data.graduation_year,
        phone: data.phone, email: data.email, company: p?.company, job_title: p?.job_title,
        region: p?.region, address: p?.address, bio: p?.bio, photo_url: p?.photo_url,
      ...
      setForm({
        company: p?.company || '', job_title: p?.job_title || '', region: p?.region || '',
        address: p?.address || '', bio: p?.bio || '', phone: data.phone || '', email: data.email || '',
        office_phone: p?.office_phone || '', fax: p?.fax || '', profile_email: p?.email || '',
  ...
  const handlePhotoCropComplete = useCallback(async () => {
    ...
    if (photoCompletedCrop && photoImgRef.current && photoCompletedCrop.width > 0) {
      const canvas = document.createElement('canvas');
      ...
      if (ctx) {
        ctx.drawImage(img, photoCompletedCrop.x * scaleX, photoCompletedCrop.y * scaleY,
  ...
  const handleCardUploadWithFile = async (file: File) => {
    ...
    try {
      ...
      const { data: ep } = await supabase.from('alumni_profiles').select('id').eq('alumni_id', id).single();
      const profileData = {
      ...
      if (ep) {
        await supabase.from('alumni_profiles').update(profileData).eq('id', ep.id);
      } else {
        await supabase.from('alumni_profiles').insert({
          alumni_id: id, company: info.company || null, job_title: info.job_title || null,
          address: info.address || null, card_image_url: url, office_phone: info.office_phone || null,
          fax: info.fax || null, email: info.email || null,
        });
      }
      await fetchData();
  ...
  const handleSave = async () => {
    ...
    const pd = {
      ...
      region: form.region || null, address: form.address || null, bio: form.bio || null,
      photo_url: form.photo_url || null, card_image_url: form.card_image_url || null,
      office_phone: form.office_phone || null, fax: form.fax || null, email: form.profile_email || null,
    };
    if (ep) await supabase.from('alumni_profiles').update(pd).eq('id', ep.id);
    else await supabase.from('alumni_profiles').insert({ alumni_id: id, ...pd });
    await fetchData(); setSaving(false); setEditMode(false); showToast('저장되었습니다');
  };

  const handleSaveContact = () => {
    if (!alumni) return;
    saveContact(alumni); setContactSaved(true); showToast('연락처를 저장합니다');
    ...
    <div style={{ ...F, minHeight: '100dvh', display: 'flex', flexDirection: 'column', background: '#f0f4f8' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '14px 14px 40px' }}>
          ...
          {editMode ? (
            ...
            <>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 0', borderBottom: '1px solid #f1f5f9' }}>
                <div style={{ flex: 1 }}><p style={{ fontSize: 10, color: '#94a3b8', marginBottom: 4 }}>소속 기관</p><OrgBadge height={18} /></div>
              </div>
        ...
        <div style={{ background: '#fff', borderRadius: 16, padding: '16px', marginBottom: 10, boxShadow: '0 1px 4px rgba(13,45,110,0.07)', border: '1px solid #e2e8f0' }}>
          ...
          {editMode
            ? <textarea value={form.bio} onChange={e => setForm(f => ({ ...f, bio: e.target.value }))} placeholder="간단한 소개를 입력해 주세요" rows={3} style={{ width: '100%', padding: '9px 12px', border: '1.5px solid #e2e8f0', borderRadius: 10, fontSize: 13, outline: 'none', resize: 'none', boxSizing: 'border-box' as const, fontFamily: 'inherit' }} />
        ...
        <div style={{ background: '#fff', borderRadius: 16, padding: '16px', marginBottom: 10, boxShadow: '0 1px 4px rgba(13,45,110,0.07)', border: '1px solid #e2e8f0' }}>
          ...
          {(editMode ? form.card_image_url : alumni.card_image_url)
            ? <img src={editMode ? form.card_image_url : alumni.card_image_url!} alt="명함" onClick={() => !editMode && setShowCard(true)} style={{ width: '100%', borderRadius: 10, border: '1px solid #e2e8f0', cursor: editMode ? 'default' : 'pointer', display: 'block' }} />
            : <div style={{ background: '#f8fafc', border: '2px dashed #e2e8f0', borderRadius: 12, padding: '28px', textAlign: 'center' }}>
              <p style={{ fontSize: 13, color: '#94a3b8' }}>{editMode ? '📷 촬영 또는 🖼 갤러리로 명함을 등록하세요\n편집 후 AI가 자동으로 정보를 저장해드려요!' : '등록된 명함이 없습니다'}</p>
            </div>}
        </div>
        ...
        {!editMode && alumni.address && (
          <div style={{ background: '#fff', borderRadius: 16, padding: '16px', marginBottom: 10, boxShadow: '0 1px 4px rgba(13,45,110,0.07)', border: '1px solid #e2e8f0' }}>
            ...
            {/* 카카오맵 roughmap - 앱 팝업 없이 웹으로 표시 */}
            <div style={{ borderRadius: 12, overflow: 'hidden', border: '1px solid #e2e8f0' }}>
              <iframe
                src={`https://map.kakao.com/?q=${encodeURIComponent(alumni.address)}&map_type=SKYVIEW&from=roughmap`}
                width="100%"
                height="240"
        ...
        {editMode && (
          <button onClick={() => {
            ...
            setForm({
              ...
              bio: alumni.bio || '', phone: alumni.phone || '', email: alumni.email || '',
              office_phone: alumni.office_phone || '', fax: alumni.fax || '',
              profile_email: alumni.profile_email || '', photo_url: alumni.photo_url || '',
              card_image_url: alumni.card_image_url || '', organization: alumni.organization || '한국농어촌공사',
            });
